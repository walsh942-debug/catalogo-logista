...
for r in reader:
    ...
    if c and p is not None: rows.append((c,d,p))
rows.sort()
...
latest=0
for p in ROOT.glob("PRECIOSTABACO*.csv"):
    m=re.fullmatch(r"PRECIOSTABACO(\d+)\.csv",p.name)
    if m: latest=max(latest,int(m.group(1)))
old=ROOT/f"PRECIOSTABACO{latest}.csv" if latest else None
if old and old.read_text(encoding="utf-8")==content:
    print("Sin cambios.")
    raise SystemExit(0)
...
print(f"PRECIOSTABACO{v}.csv: {len(rows)} productos")
